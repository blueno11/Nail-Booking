...
<head>
    ...
    <title>Quản lý Thông báo - Nailology Admin</title>
    ...
    <style>
        ...
        .content-preview { max-width: 300px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; color: #666; font-size: 0.875rem; }
    </style>
</head>
<body>
...
<div class="container admin-container">
    <div class="admin-nav">
        ...
        <a href="${pageContext.request.contextPath}/admin/announcements" class="active">Thông báo</a>
        <a href="${pageContext.request.contextPath}/admin/staff">Nhân viên</a>
    </div>

    <div class="admin-header">
        <h1 class="admin-title">Quản lý Thông báo</h1>
        <a href="${pageContext.request.contextPath}/admin/announcements/edit?id=0" class="btn btn-primary">+ Thêm thông báo</a>
    </div>
        ...
        <tbody>
            <c:forEach items="${announcementList}" var="ann">
                <tr>
                    ...
                    <td class="content-preview">${ann.content}</td>
                    <td>
                        <c:if test="${ann.startDate != null}">
                            <fmt:formatDate value="${ann.startDate}" pattern="dd/MM/yyyy"/>
                        </c:if>
                        <c:if test="${ann.startDate != null && ann.endDate != null}"> - </c:if>
                        <c:if test="${ann.endDate != null}">
                            <fmt:formatDate value="${ann.endDate}" pattern="dd/MM/yyyy"/>
                        </c:if>
                        <c:if test="${ann.startDate == null && ann.endDate == null}">
                            <em style="color:#999">Không giới hạn</em>
                        </c:if>
                    </td>
                    <td>
                        <span class="badge ${ann.active ? 'badge-success' : 'badge-secondary'}">
                            ${ann.active ? 'Đang hiển thị' : 'Ẩn'}
                        </span>
                    </td>
                    <td>${ann.priority}</td>
                    <td class="action-links">
                        <a href="${pageContext.request.contextPath}/admin/announcements/edit?id=${ann.id}">Sửa</a>
                        <a href="${pageContext.request.contextPath}/admin/announcements/delete?id=${ann.id}" 
                           class="delete-link"
            ...
            <c:if test="${empty announcementList}">
                <tr><td colspan="7" style="text-align:center; padding:2rem; color:#999;">Chưa có thông báo nào.</td></tr>
            </c:if>
        </tbody>
    </table>
